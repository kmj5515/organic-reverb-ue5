...
- 리버브 파라미터(Decay 등)는 전역 Submix 하나에 적용되므로 모든 소리가 리스너 방의 잔향 성격을 받는다. 음원마다 다른 것은 잔향 Send 양뿐이다 (`Organic Sound Source` 컴포넌트, `PlayGunshotAtLocation`)
